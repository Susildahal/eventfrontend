...
export async function GET(req: NextRequest) {
  ...
  // Proxy GET /api/profile -> backend /profile
  return proxyFetchWithReq(`/profile${search}`, req)
}

export async function POST(req: NextRequest) {
  const body = await req.json()
  return proxyFetchWithReq('/profile', req, {
    method: 'POST',
...
export async function PUT(req: NextRequest) {
  ...
  const body = await req.json()
  const target = id ? `/profile/${id}` : (body.id ? `/profile/${body.id}` : '/profile')
